...
void HC05_init(void)
{
    Uart_Init(UART5, 9600, SYSCLKMODE);
    Uart_Control(UART5, TRANSMITFLAG, !RECEIVEFLAG, WLEN8);
    DIO_InitClock(PORTE);
    DIO_DigitalEnable(PORTE);
   
    GPIO_AlternateFunc(PORTE, PIN4, 1); 
    GPIO_AlternateFunc(PORTE, PIN5, 1);        
}

char Bluetooth_Read(void)  
{
    return Uart_RX(UART5); 
}
